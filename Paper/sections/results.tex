...
one basis point tightening surprise, averaged across the eleven countries.
It comes from the specification without the interaction term, so the
coefficient is the common response of all countries. The
...
adjustable rate indicator at horizons of zero, four and eight quarters,
that is on impact, after one year and after two years. These three
horizons are only conventional summary points. The local projections
impose no restrictions across horizons, so no single horizon is special,
and Figure \ref{fig:interaction} therefore plots the complete path over
all twelve quarters.
...
\subsection{A within quarter check}
\label{sec:timefe}

The absence of time fixed effects is the main cost of the country level
design, because everything that moves all euro area housing markets at
once ends up in the error term. A simple check turns this around.
Dropping the main effect of the shock and adding time fixed effects
absorbs the common component of house prices entirely, together with the
shock itself and its lags, and the interaction is then identified from
the within quarter difference between variable rate and fixed rate
countries. Table \ref{tab:timefe} in the appendix reports this
specification at the same three horizons. The interaction stays small and
positive, $0.018$ with a standard error of $0.011$ on impact and far from
significance at the longer horizons, and the impact estimate now loses
even its marginal ten percent significance. Absorbing every common shock
therefore leaves the conclusion from Table \ref{tab:main} unchanged.
